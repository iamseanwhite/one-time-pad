	...
	while(1){
		...
		switch(spawnpid) {
	        ...
	        case 0: {
				...
				int compareReturn = strncmp(buffer, "enc", 3);
				...
				fflush(stdout);
					...
					//printf("New buffer after enc taken out: %s\n", plaintextPlusKey);
				...
				fflush(stdout);	
					...
					int plainNum[strlen(buffer)];
					int keyNum[strlen(buffer)];
					int cyphertextNum[strlen(buffer)];
					char temp;
					int j;
					
					//build plainNum array
					...
					//build cyphertextNum
					for (i = 0; i < strlen(buffer); i++) {		
						cyphertextNum[i] = plainNum[i] + keyNum[i];
						if (cyphertextNum[i] >= 27) {					//% 27
							cyphertextNum[i] -= 27;
						}
					}
					
					
					//build cyphertext string from int array
					char cyphertext[strlen(buffer)];
					memset(cyphertext, '\0', sizeof(cyphertext));
					...
					for (i = 0; i < strlen(buffer); i++) {
						...
						for (j = 0; j < strlen(alphabet); j++) {
							...
							if (cyphertextNum[i] == j){
								cyphertext[i] = alphabet[j];
							}
						}
